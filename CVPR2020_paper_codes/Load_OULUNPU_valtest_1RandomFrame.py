...
class ToTensor_valtest(object):
    ...
    def __call__(self, sample):
        ...
        return {'image_x': torch.from_numpy(image_x.astype(float)).float(), 'val_map_x': torch.from_numpy(val_map_x.astype(float)).float(),'spoofing_label': torch.from_numpy(spoofing_label_np.astype(np.int64)).long()} 
...
class Spoofing_valtest(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # image_x, val_map_x = self.get_single_cropped_image_x(image_path, val_map_path, videoname)
    ...
    # Bernardo
    def get_single_cropped_image_x(self, image_path, val_map_path, videoname):
        ...
        image_pattern = image_path + '*.png'
        found_image_path = glob.glob(image_pattern)
        assert len(found_image_path) > 0, f'Error, no file found with pattern \'{image_pattern}\''
        found_image_path = found_image_path[0]
        image_x_temp = cv2.imread(found_image_path)
        
        # gray-map
        # val_map_x_temp = cv2.imread(val_map_path2, 0)
        map_pattern = val_map_path + '*depth.png'
        found_map_path = glob.glob(map_pattern)
        assert len(found_map_path) > 0, f'Error, no file found with pattern \'{map_pattern}\''
        found_map_path = found_map_path[0]
        val_map_x_temp = cv2.imread(found_map_path, 0)
        ...
        # val_map_x[ii,:,:] = temp
        val_map_x = cv2.resize(val_map_x_temp, (32, 32))

        image_x = np.expand_dims(image_x, axis=0)
        val_map_x = np.expand_dims(val_map_x, axis=0)
    ...
    def get_single_image_x(self, image_path, val_map_path, videoname):
        ...
        for ii in range(frames_total):
            ...
            for temp in range(50):
                # s = "_%03d_scene" % image_id   # original
                s = "_%1d" % image_id            # Bernardo

                # s1 = "_%03d_depth1D" % image_id   # original
                s1 = "_%1d" % image_id              # Bernardo
